...
window.onload = function () {
    ...
    const worker = new Worker('./js/wkrSocket.js');
    ...
    function graficarPunto(data){               
        let coordX = data.coordenadaX;
        let coordY = data.coordenadaY;
        let color = data.color;                

        if (coordX != "H"){
            ctx.beginPath();
            ctx.arc(coordX, coordY, 1, 0, 2 * Math.PI);
